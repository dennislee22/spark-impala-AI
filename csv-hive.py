...
try:
    print(f"\nWriting data to Hive table: {HIVE_DATABASE}.{HIVE_TABLE_NAME}...")
    df_transformed.write \
        .mode("overwrite") \
        .saveAsTable(f"{HIVE_DATABASE}.{HIVE_TABLE_NAME}")

    print(f"Successfully wrote data to Hive table: {HIVE_DATABASE}.{HIVE_TABLE_NAME}")
    print(f"\nVerifying data in Hive table: {HIVE_DATABASE}.{HIVE_TABLE_NAME}")
    df_hive = spark.table(f"{HIVE_DATABASE}.{HIVE_TABLE_NAME}")
    df_hive.show(5)
    print("Verification complete.")

except Exception as e:
    print(f"Error writing to Hive table: {e}")
